...
class SimpleTokenizer:
    ...
    def __init__(
        ...
        if tokenizer_type == "char":
            ...
            common_words = [
                ...
                'that', 'these', 'those', 'he', 'she', 'it', 'they', 'we', 'you',
                'I', 'me', 'him', 'her', 'us', 'them', 'my', 'your', 'his', 'our',
                'student', 'write', 'read', 'learn', 'study', 'test', 'good', 'bad',
                'well', 'better', 'best', 'English', 'language', 'word', 'sentence',
                '.', ',', '!', '?', "'", '"', '-', ':', ';'
            ]
            special_tokens = ['<PAD>', '<UNK>', '<BOS>', '<EOS>']
    ...
    def from_json(cls, data: Dict):
        ...
        tokenizer.id_to_token = {int(k): v for k, v in {idx: token for token, idx in tokenizer.token_to_id.items()}.items()}
...
def create_mock_model_and_tokenizer(
    ...
    print(f"Creating mock language model...")
    ...
    print(f"  python -m src.extract_perplexity_features \\")
    print(f"    --text \"The student writes well.\" \\")
    print(f"    --model-type pytorch \\")
    print(f"    --model {model_path} \\")
    print(f"    --tokenizer-path {tokenizer_path} \\")
    print(f"    --output test_perplexity.json")
