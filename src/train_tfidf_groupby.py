...
experiment-dir/
└── ml-training-data/
    └── train-data.csv              # Must have 'text' and 'cefr_label' columns

OUTPUT:
experiment-dir/
└── feature-models/
    └── tfidf/
        └── {hash}_tfidf_grouped/   # Hash based on TF-IDF config
            ├── tfidf_model.pkl     # Trained GroupedTfidfVectorizer
            └── config.json         # TF-IDF configuration metadata

The GroupedTfidfVectorizer creates N separate TF-IDF models (one per CEFR level)
and concatenates their outputs into a single feature vector.
"""
